...
class MultiSpanHead(Module):
    ...
    def decode_spans_from_tags(
        tags, question_passage_tokens, passage_text, question_text
    ):
        spans_tokens = []
        prev = 0  # 0 = O
        ...
        spans_text, spans_indices = decode_token_spans(
            valid_tokens, passage_text, question_text
        )
...
class SimpleBIO(MultiSpanHead):
    ...
    def log_likelihood(self, gold_labels, log_probs, seq_mask, is_bio_mask, **kwargs):
        ...
        log_likelihoods_for_multispan = torch.gather(
            log_probs, dim=-1, index=gold_labels.unsqueeze(-1)
        ).squeeze(-1)

        # Our marginal likelihood is the sum of all the gold label likelihoods, ignoring the
        # padding tokens.
        log_likelihoods_for_multispan = replace_masked_values(
            log_likelihoods_for_multispan, seq_mask, 0.0
        )
        ...
        log_marginal_likelihood_for_multispan = replace_masked_values(
            log_marginal_likelihood_for_multispan, is_bio_mask, -1e7
        )
    ...
    def prediction(self, log_probs, logits, qp_tokens, p_text, q_text, mask):
        ...
        return MultiSpanHead.decode_spans_from_tags(
            predicted_tags, qp_tokens, p_text, q_text
        )
...
class FlexibleLoss(MultiSpanHead):
                ...
                if answer_as_text_to_disjoint_bios.sum() > 0:
                    ...
                    if self._generation_top_k > 0:
                        ...
                        most_likely_predictions = (
                            most_likely_predictions * seq_mask.unsqueeze(1)
                        )
    ...
    def _filter_correct_predictions(
        self, predictions, answer_as_text_to_disjoint_bios, full_bio
    ):
        ...
        expanded_predictions = (
            ...
            .unsqueeze(2)
            .repeat(1, 1, texts_count, spans_count, 1)
        )
        expanded_answer_as_text_to_disjoint_bios = (
            answer_as_text_to_disjoint_bios.unsqueeze(1)
        )
        expanded_full_bio = full_bio.unsqueeze(1).unsqueeze(-2).unsqueeze(-2)

        disjoint_intersections = (
            expanded_predictions == expanded_answer_as_text_to_disjoint_bios
        ) & (expanded_answer_as_text_to_disjoint_bios != 0)
        some_intersection = disjoint_intersections.sum(-1) > 0
        only_full_intersections = (
            (
                (expanded_answer_as_text_to_disjoint_bios != 0) - disjoint_intersections
            ).sum(-1)
            == 0
        ) & (expanded_answer_as_text_to_disjoint_bios.sum(-1) > 0)
        ...
        correct_mask &= (
            (
                (expanded_full_bio != expanded_predictions)
    ...
    def _add_full_bio(self, correct_most_likely_predictions, full_bio):
        ...
        not_added = (
            (full_bio.unsqueeze(1) == correct_most_likely_predictions).prod(-1).sum(-1)
            == 0
        ).long()

        return torch.cat(
            (
                correct_most_likely_predictions,
    ...
    def take_step(
        ...
        transitions_mask[:, 1:3] &= (
            ((class_log_probabilities[:, :3] == 0.0).sum(-1) != 3)
            .unsqueeze(-1)
            .repeat(1, 2)
        )

        # assuming the wordpiece mask doesn't intersect with the other masks (pad, cls/sep)
        transitions_mask[:, 2] |= is_wordpiece & (
...
multispan_heads_mapping = {
    "simple_bio": SimpleBIO,
    "crf_loss_bio": CRFLossBIO,
    "flexible_loss": FlexibleLoss,
}
