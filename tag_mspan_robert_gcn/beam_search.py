...
class BeamSearch:
    ...
    def search(
        ...
        ) = start_class_log_probabilities.topk(
            min(self.beam_size, start_class_log_probabilities.size()[-1])
        )  # argument to topk was self.beam_size
        ...
        if self.beam_size > start_class_log_probabilities.size()[-1]:
            ...
            start_top_log_probabilities = torch.cat(
                (x, x.new_full((x.size()[0], self.beam_size - x.size()[-1]), -1e7)),
                dim=-1,
            )
            start_predicted_classes = torch.cat(
                (
                    y,
                    y.new_full(
                        (x.size()[0], self.beam_size - y.size()[-1]),
                        start_predictions[0],
                    ),
                ),
                dim=-1,
